...
Play chords with your left hand, pulse them with your right — like fretting
a guitar and giving it rhythmic hits instead of a steady strum. No mic
needed — just your camera.
...
- Tilt that hand to the left to force whichever chord you're currently
  ...
  quality.
- **Right hand**: raise it above chest height to pulse the held chord —
  "DO DO DO DO" instead of one continuous "DOOOOO". Lower it and the chord
  goes back to sounding as one steady tone.
...
- `hands.js` — reads finger shapes, hand tilt, and whether the right hand is raised, from the camera (via MediaPipe)
- `audio-engine.js` — the synth: a held pad for the chord, gated on and off rhythmically for the pulse (via Tone.js)
...
- Good, even lighting and both hands inside the frame help hand tracking
  a lot.
- The finger-shape recognition, the tilt threshold, and the pulse
