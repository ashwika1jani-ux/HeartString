...
// left hand's chord plays at the normal octave, the right hand's chord
// plays independently one octave higher. Either hand can tilt left to
// flip its own chord to the opposite of its normal quality.

const HandTracker = (() => {
  let hands = null;
  let camera = null;
  let videoEl = null;
  let mirrorFlip = true; // toggled from the UI if left/right ever comes out backwards
  ...
  const state = {
    ...
    tiltLeft: false,        // true while the left hand is tilted — flips its chord's quality
    tiltRight: false,       // true while the right hand is tilted — flips its chord's quality
    ready: false
  };
  ...
  function processResults(results) {
    ...
    state.tiltLeft = false;
    state.tiltRight = false;
    ...
    if (state.screenLeft) {
      ...
      state.tiltLeft = angle > TILT_THRESHOLD_RAD;
    }

    // --- right hand: same chord shapes and tilt, played an octave up ---
    if (state.screenRight) {
      const fingers = fingerStates(state.screenRight.landmarks);
      state.degreeIndexRight = shapeToDegree(fingers);
      const angle = tiltAngle(state.screenRight.landmarks);
      state.tiltRight = angle > TILT_THRESHOLD_RAD;
    }
  }
