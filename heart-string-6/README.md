...
- Tilt either hand to the left to flip whichever chord it's currently
  shaping to the opposite of its normal quality — major becomes minor,
  minor (or the 7th's diminished chord) becomes major. Straighten back up
  and it returns to normal. The key itself never changes from this, only
  that one chord's quality.
...
- `hands.js` — reads finger shapes and tilt on both hands, from the camera (via MediaPipe)
